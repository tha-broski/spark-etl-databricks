...
def load_snapshot_to_silver(
    spark, source_df, silver_table, snapshot_ids, entity_id_column, compare_columns
):
    ...
    if not spark.catalog.tableExists(silver_table):
        ...
        source_df.write.format("delta").mode("append").saveAsTable(silver_table)
        return

    # No first load -> load Silver table as DeltaTable for MERGE
...
def load_incremental_to_silver(
    spark, source_df, silver_table, entity_id_column, compare_columns
):
