...
def mark_file_status(spark, source_file, file_hash, batch_id, status, control_table):
    ...
    processed_file.write.format("delta").mode("append").saveAsTable(control_table)


def get_file_state(spark, file_hash, control_table):
...
def prepare_file_batch(spark, source_path, schema, bronze_table, control_table):
    ...
    file_state = get_file_state(spark, file_hash, control_table)
    ...
    # File hasn't been processed yet -> append it to Bronze | batch_id generated in ingestion/bronze.py
        ...
        # Save intermediate state so retries do not duplicate Bronze data
        mark_file_status(
            spark,
            source_path,
            file_hash,
